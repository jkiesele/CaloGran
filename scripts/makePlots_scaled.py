...
charged_pion='isPionCharged'

def charged_pion_energy(variable='reg_E/true_energy - 1', xaxisname='E_{pred}/E_{true}-1',  outadd='', xrange=0.3, nbins=37):
    makePlots_async(infile,
                ['E = 10 GeV',
                 ...
                 'E = 500 GeV',
                 'E = 1000 GeV',
                 'E = 2000 GeV'], #legend names [as list]
                
                8*[variable], #variable to plot --> yaxis:xaxis
                [charged_pion+'&& abs(true_energy - 10)<2.5',
                 charged_pion+'&& abs(true_energy - 20)<2.5',
                 charged_pion+'&& abs(true_energy - 50)<5',
                 charged_pion+'&& abs(true_energy - 100)<5',
                 charged_pion+'&& abs(true_energy - 200)<10',
                 charged_pion+'&& abs(true_energy - 500)<25',
                 charged_pion+'&& abs(true_energy - 1000)<100',
                 charged_pion+'&& abs(true_energy - 2000)<100'], #list of cuts to apply
                'auto', #list of color and style, e.g. ['red,dashed', ...]
                outdir+'/resPlot_'+charged_pion+"_" +outadd+'.pdf', #output file (pdf)
                xaxisname, #xaxisname
                'A.U.', #yaxisname
                normalized=False,
                profiles=False,
...
charged_pion_energy()
charged_pion_energy(outadd='finebins',nbins=100)
charged_pion_energy(outadd='largex',xrange=1)
charged_pion_energy(outadd='largex_finebins',xrange=1,nbins=100)
charged_pion_energy('sqrt((reg_E/true_energy - 1)*(reg_E/true_energy - 1))','RMS','RMS')
...
for part in truthclasses:
    ...
    ##energy resolution plot
        ...
        makePlots_async(infile,
                ...
                7*['reg_E/true_energy - 1'], #variable to plot --> yaxis:xaxis
                [part+'&& abs(true_energy - 10)<2.5',
                 part+'&& abs(true_energy - 20)<2.5',
                 part+'&& abs(true_energy - 50)<5',
                 part+'&& abs(true_energy - 100)<5',
                 part+'&& abs(true_energy - 200)<10',
                 part+'&& abs(true_energy - 500)<25',
